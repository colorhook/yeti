/**
 * @version 1-0-1
 * @modify zhengxie.lj@taobao.com
 * @description 修改测试方式，支持远程测试。
 */
...
function serveExpress (port, path, cb) {
	...
	/**
	 * @data 2011-3-8
	 * @description 为k2架构的目录设置代理
	 */
	var proxy_http = function(options, callback){
		var host = options.host || 'localhost';
		var headers = {
			  host: host
		};
		var proxy = http.createClient((options.port || 80), host);
		var req = proxy.request('GET', options.url, headers);
		req.end();
	...
	var redirectArr = ['/k2/*', '/labs/*', '/product/*', '/promotion/*', '/global/*', '/yui/*', '/voc/*'];
	var proxy_req_res = function(req, res){
		var headers = req.headers,
					addr = req.connection.remoteAddress;
					headers.host = req.connection.remoteAddress;
				...
				proxy_http({
					...
					var reg =/\.(png|gif|jpg|jpeg)$/i;
					var matches = req.url.match(reg);
					console.log(matches);
					if(matches && matches.length>=2){
						var m = matches[1];
						res.header("Content-Type", "image/"+m.toLowerCase());
	...
	(function(){
		for(var i = 0, l = redirectArr.length; i < l; i++){
			app.get(redirectArr[i], function(req, res){
				proxy_req_res(req, res);
			});
		}
	})();
	/////////////////////////////////////
    app.get('/project/*', function (req, res) {
		 ...
		 * @data 2011-3-8
		 * @description 
		 * 如果是来自ATT的测试文件，则通过远程请求html的方式。
		 * 1. 避免使用临时文件来测试
		 * 2. 支持php
		 */
		 if(/yeti-proxy/.test(req.url)){
			 if(/^.*\.(html|php)?$/.test(req.url)){
				 (function(){
					 var remoteFile = file.replace(/.*yeti-proxy/, "");
					 ...
					 proxy_http({
						...
						var yetify =  "<script src=\"/dyn/" + cachebuster
					+ "/inject.js\"></script><script>"
					+ "$yetify({url:\"/results\"});</script>";
						res.send(data+yetify);
					});
				 })();
			 }else{
				 proxy_req_res(req, res);
